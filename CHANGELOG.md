...
- **Must-fail arms, each run by mutating the file, running the full suite, and restoring:**
  ...
  - Break A2, `judgeRead` returns `null` and `judgeWith` is patched to treat null as "go on", so every judge still works and only the null is under test: 3 failures, all three PRESENT_UNCHECKED tests.
...
- **What this does not guarantee.** The binding proves a green came out of a judge over some `got` object that was handed to `judgeRead`. It does not prove that `got` came off the network: a caller can still pass `{json: <anything>}` to a judge. That is what `api/pin.js`'s pointer rebuild and the fresh-stamp path do legitimately, and the shape rules still run on it. `refusal()` of a `present_unchecked` renders the ordinary 503, which fails closed.
