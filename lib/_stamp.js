...
// A record read back from the store only counts as the stamp for `sha` when it
// is a plain object whose own sha256 (lowercased) IS `sha`. The path is only
// where we looked; the record has to say the same thing. Anything else is a
// store integrity problem, never a "stamped".
function recordMatches(json, sha) {
  return !!json && typeof json === "object" && !Array.isArray(json) &&
    typeof json.sha256 === "string" && json.sha256.toLowerCase() === sha;
}

function mismatch(res, sha) {
  console.error(`[stamp] integrity: record at ${stampPath(sha)} does not match its fingerprint`);
  return res.status(409).json({
    ok: false, stamped: false, sha256: sha, scope: SCOPE, reason: "record_mismatch",
    error: "the record at this path does not match its fingerprint; this is a store integrity problem, not a stamp",
  });
}
...
function present(record, { existing, commitSha, billing }) {
...
async function handleStamp(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!recordMatches(found.json, sha)) return mismatch(res, sha);
  ...
  try {
    ...
    if (prior) {
      if (!recordMatches(prior.json, v.sha256)) return mismatch(res, v.sha256);
      return res.status(200).json(present(prior.json, { existing: true }));
    }
    ...
    if (put && put.abandoned) {
      ...
      if (theirs) {
        if (!recordMatches(theirs.json, v.sha256)) return mismatch(res, v.sha256);
        return res.status(200).json(present(theirs.json, { existing: true }));
      }
    ...
    return res.status(201).json(present(record, { existing: false, commitSha, billing }));
  } catch (e) {
