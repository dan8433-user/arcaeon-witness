...
function judgeStamp(record, sha) {
  ...
  if (!record || typeof record !== "object" || Array.isArray(record)) return verdict.red(what, "not_a_json_object");
  if (record.sha256 !== sha) return verdict.red(what, "sha256_mismatch");
  if (typeof record.stamped_at !== "string" || !Number.isFinite(Date.parse(record.stamped_at))) return verdict.red(what, "stamped_at_unreadable");
  return verdict.verifiedRecord(what);
}
...
function sendStamp(res, status, record, sha, opts) {
  const v = judgeStamp(record, sha);
  if (!v.ok) {
    const refused = verdict.refusal(v, "stamp");
    return res.status(refused.status).json({ ...refused.body, sha256: sha });
  }
  return res.status(status).json(present(v, record, opts));
}
...
async function takeDailyBudgetOnce(nowIso, ceiling = DAILY_CAP) {
  ...
  await stampStore.putFile(
    ...
    {
      rebuild: (fresh) => {
        // putFile hands the rebuild hook bare JSON, or null when the path is gone.
        const n = countOf(fresh === null ? null : { json: fresh });
        if (n >= ceiling) {
          denied = denial(n);
          return null;
        }
...
async function handleStamp(req, res) {
  ...
  if (req.method === "GET") {
    ...
    return sendStamp(res, 200, found.json, sha, { existing: true });
  }
  ...
  try {
    ...
    if (prior) return sendStamp(res, 200, prior.json, v.sha256, { existing: true });
    ...
    if (put && put.abandoned) {
      ...
      if (theirs) return sendStamp(res, 200, theirs.json, v.sha256, { existing: true });
    ...
    return sendStamp(res, 201, record, v.sha256, { existing: false, commitSha, billing });
  } catch (e) {
