...
//      data but new tests should not use them. The three 2026-08-14 audit
//      fixtures `velouria-audit1..3` are recognised by LEGACY_TEST_PATTERNS
//      (`velouria-audit` + digits and nothing else). A bare `velouria-audit`
//      prefix is deliberately NOT used: `velouria-audit-20260819` is a live
//      namespace and must never read as test data.
...
// Whole-name patterns (anchored both ends) for legacy test names that share a
// stem with a live namespace, so a prefix would be too wide.
const LEGACY_TEST_PATTERNS = [/^velouria-audit\d+$/];
...
function isTestNamespace(ns) {
  return isReservedTestNamespace(ns) ||
    (typeof ns === "string" &&
      (LEGACY_TEST_PREFIXES.some((p) => ns.startsWith(p)) ||
       LEGACY_TEST_PATTERNS.some((re) => re.test(ns))));
}
