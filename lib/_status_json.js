...
const { gatherStatusData, overallWord } = require("./_status_data.js");
...
module.exports = async (req, res) => {
  ...
  const out = {
    ...
    // What a relying party needs to know about THIS SERVICE, stated as a
    // property of the service (2026-09-19; replaces a note that described who
    // was using it). Update `witness_operators` only when a second independent
    // witness is actually deployed and verified; that claim stays behind the
    // publish gate until then.
    independence: {
      roots: 1,
      witness_operators: 1,
      verifiable_without_trusting_us: true,
      note: "single-operator witness; every pin is a public commit that can be checked without trusting this service; commit dates are the operator's own clock, the daily Bitcoin (OpenTimestamps) anchor is the independent one",
    },
    namespaces,
    conflict_observations: {
      count: data.obsCount,
      sample: data.obsSample.map((o) => ({ path: o.path, record: o.obs, url: o.url })),
      note: data.obsCount === null
        ? "conflicts could NOT be counted on this render (see errors.observations) — null here means unknown, never zero"
        : data.obsCount > data.obsSample.length
