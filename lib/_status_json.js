...
module.exports = async (req, res) => {
  ...
  const out = {
    ...
    // Independence disclosure — a count of independent derivation ROOTS, not
    // of signatures. Every namespace on this witness today traces to one
    // operator root; the determinate number ("k=1, disclosed") beats an
    // indeterminate impression, per an external reviewer's critique. Update
    // this ONLY when a namespace genuinely under someone else's root exists —
    // and that claim stays behind the publish gate until deployed+verified.
    independence: {
      roots: 1,
      disclosed: true,
      note: "all namespaces trace to one operator root; k counts roots, not signatures",
    },
    namespaces,
