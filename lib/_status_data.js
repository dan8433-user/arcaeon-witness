...
async function gatherStatusData() {
  ...
  const indeterminate = !degraded && (ungradeableCount > 0 || anchorUnknown || nothingWatched);
...
// namespace is simply untagged. Override with WITNESS_REFERENCE_NS_PREFIXES.
const REFERENCE_PREFIXES = (process.env.WITNESS_REFERENCE_NS_PREFIXES || "velouria-,arcaeon-,test-,demo-")
  .split(",").map((x) => x.trim()).filter(Boolean);
function isReferenceNamespace(ns) {
  return REFERENCE_PREFIXES.some((pre) => String(ns).startsWith(pre));
}

module.exports = {
  isReferenceNamespace, gatherStatusData, cadenceStatus, humanDuration, BLOB, TREE, COMMITS, REPO_URL };
