...
.copyblock{position:relative;margin:14px 0}
.copyblock pre{margin:0;border-color:var(--gold);padding-right:86px}
.copyblock button.copy{position:absolute;top:10px;right:10px}
.pstat{display:block;min-height:1.3em;margin:8px 0 0;font-size:.85rem}
.pstat.idle{color:var(--muted)}
.pstat.checking{color:var(--muted)}
.pstat.free{color:#8fe3a8}
.pstat.taken{color:#ff9d9d}
.pstat.unknown{color:var(--gold-soft)}
#prefix-alts{display:flex;flex-wrap:wrap;align-items:center;gap:8px;margin-top:8px}
button.alt{background:transparent;color:var(--gold-soft);border:1px solid var(--line);border-radius:999px;padding:4px 12px;font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;font-size:.82rem;cursor:pointer}
...
// Multi-line sibling of copyBox: a <pre> that keeps its newlines with the
// same copy button floated into the corner. The curl pin command is four
// lines and was previously a bare <pre> the buyer had to select by hand —
// which is exactly where a half-selected command comes from.
// The copy handler is the same one in pageShell (it reads textContent), so a
// copied command carries real newlines, not the rendered escapes.
function copyBlock(id, text) {
  return `<div class="copyblock"><pre id="${esc(id)}">${esc(text)}</pre><button type="button" class="copy" data-copy-target="${esc(id)}">Copy</button></div>`;
}

module.exports = { SUPPORT_EMAIL, docsUrl, baseUrl, esc, wantsJson, pageShell, copyBox, copyBlock };
