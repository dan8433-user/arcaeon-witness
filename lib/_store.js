...
//       FRESH store read ({json, sha}, or null on a 404 only) and decides what to write on top of it; returning null
...
async function putFile(path, obj, message, sha, opts = {}, target = PINS_TARGET) {
  ...
  while (attempts < PUT_RETRY.maxAttempts) {
    ...
    // The hook gets the WHOLE store read, exactly as getFile returned it:
    // null only for a 404, {json, sha} for anything present — including a
    // present file whose JSON is the literal `null`. Until 2026-09-22 this
    // passed `fresh ? fresh.json : null`, which folded "gone" and "present
    // but null" into one value, and api/pin.js's pointer hook judged that
    // value as a 404 and rebuilt over it at seq 0. The hook judges the read.
    const next = await rebuild(fresh, freshSha);
    if (next === null || next === undefined) {
      return {
        abandoned: true,
        attempts,
        fresh: fresh ? fresh.json : null,
        // Informational only (no caller reads it): whether the path was
        // present, so `fresh: null` never has to stand for both states.
        fresh_present: fresh !== null,
        fresh_sha: freshSha,
      };
    }
    body = next;
...
async function listDir(path, target = PINS_TARGET) {
  ...
  if (!r.ok) throw new Error(`github GET ${path} -> ${r.status}`);
  const body = await r.json();
  // [] is reachable by exactly one road: the 404 above. A 200 that is not a
  // listing is unreadable, not empty (lib/_verdict.js).
  return verdict.requireListing(body, `github LIST ${path}`);
}
...
async function getTree(target = PINS_TARGET) {
  ...
  const body = await r.json();
  return verdict.requireListing(body && body.tree, "github GET tree");
}
...
async function getTreeMeta(target = PINS_TARGET) {
  ...
  const body = await r.json();
  return {
    entries: verdict.requireListing(body && body.tree, "github GET tree"),
...
function computeCadenceFields(pin, now = Date.now()) {
  ...
  if (!pin || typeof pin !== "object" || Array.isArray(pin)) {
    throw new verdict.VerdictRequiredError("computeCadenceFields: a pin record", pin);
  }
