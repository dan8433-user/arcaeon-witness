...
//
// Generalized over the PATH (2026-09-20) so lib/_claim.js can use the same one
// CAS primitive the pending document already runs on, rather than carrying a
// second copy of this fetch pair that can drift from it. The pending-specific
// wrappers below are unchanged in behaviour and in signature.
async function readUsageDoc(path) {
  const r = await fetch(`${API}/repos/${USAGE_REPO}/contents/${path}?ref=${USAGE_BRANCH}`, {
    headers: ghHeaders(),
  });
  if (r.status === 404) return null;
  if (!r.ok) {
    console.error(`[pending] GET ${path} -> ${r.status}`);
...
//
// The two shapes are the two halves of the only mutual-exclusion primitive this
// store has, and lib/_claim.js leans on BOTH: an omitted sha is
// create-if-absent (422 = somebody else created it first), a supplied sha is
// update-if-unchanged (409 = somebody else moved it first).
async function writeUsageDoc(path, state, sha, message) {
  const payload = {
    message,
    branch: USAGE_BRANCH,
  ...
  if (!r.ok) {
    ...
    console.error(`[pending] PUT ${path} -> ${r.status}: ${detail.slice(0, 300)}`);
...
async function readPending() {
  return readUsageDoc(PENDING_PATH);
}

async function writePending(state, sha, message) {
  return writeUsageDoc(PENDING_PATH, state, sha, message);
}
