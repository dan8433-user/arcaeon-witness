...
//
// Two damaged shapes used to read as green here (VERDICT_SURVEY.md §6):
//   - a `sealing` slot that is PRESENT but has no leaf array (or no integer
//     batch_id) read as "nothing owed", and the close below then wrote a new
//     slot over it, so any unpublished leaves in it were gone with no refusal.
//     Only null/absent means "no slot" (initialState writes null); anything
//     else must be a well-formed slot or the run refuses as corrupt.
//   - a root.json that is PRESENT but carries no root read as "published", so
//     the slot's leaves were dropped from the carry on the strength of a file
//     that says nothing. A published root has to say what it published.
...
async function resolveSealing(state) {
  const slot = state.sealing;
  if (slot === null || slot === undefined) return { carry: [], published: false };
  if (typeof slot !== "object" || Array.isArray(slot) || !Array.isArray(slot.leaves)) {
    throw corrupt("pending: sealing slot is present but has no leaf array");
  }
  if (slot.leaves.length === 0) return { carry: [], published: false };
  const id = slot.batch_id;
  if (!Number.isInteger(id) || id < 1) {
    throw corrupt("pending: sealing slot holds leaves but no integer batch_id to resolve them against");
  }
  const rec = await store.getFile(batch.rootPath(id));
  if (rec) {
    const root = rec.json && typeof rec.json === "object" ? rec.json.root : undefined;
    if (typeof root !== "string" || root === "") {
      // Not `corrupt`: the damage is in the PIN repo, not the pending document,
      // so sealOnce reports it as pin_repo_unreadable. Refusing is the only
      // safe answer: re-sealing could publish these leaves twice, dropping them
      // could lose them.
      throw new Error(`sealer: ${batch.rootPath(id)} is present but carries no root — cannot tell whether batch ${id} published`);
    }
    return { carry: [], published: true, batch_id: id, root };
  }
  return { carry: pending.validateLeaves(slot.leaves, "sealing.leaves"), published: false, batch_id: id };
}
